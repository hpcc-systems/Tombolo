...
function Files({hpcc_queryid, cluster_id}) {
  ...
  const getQueryFiles = async () =>{
    try{
        ...
        const response = await fetch(`/api/hpcc/read/getQueryFiles?hpcc_queryId=${hpcc_queryid}&clusterId=${cluster_id}`, {headers: authHeader()});
        ...
        logicalFiles = logicalFiles.map(logicalFile => ({key: logicalFile, fileName: logicalFile, fileType: 'Logical File'}));
        superFiles = superFiles.map(superFile => ({key : superFile.title, fileName : superFile.title, fileType : 'Super File', 
