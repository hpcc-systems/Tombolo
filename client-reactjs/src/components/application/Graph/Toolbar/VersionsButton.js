...
import { CaretRightOutlined, CloudDownloadOutlined, DeleteOutlined, EditOutlined, ExclamationCircleOutlined, LoadingOutlined, PauseOutlined, RollbackOutlined, SaveOutlined } from '@ant-design/icons';
import { Modal, Form, Input, Empty,  notification, Button, message, Alert, Space, Typography, Divider, Badge, Tag } from 'antd';
...
const VersionsButton = ({ graphRef }) => {
  ...
  const promptToSwitchVersion = (e, version) => {
    ...
    confirm({
      ...
      okText:'Switch',
      onOk() { return switchVersion(version, true ); },
      onCancel() {},
    });
  };
  ...
  const switchVersion = async (version, isPermanent = false ) =>{
    try {
      ...
      graphRef.current.unfreeze().enableSelection().enableRubberband().enableSnapline().enableSharpSnapline().hideTools();
      ...
      if (isPermanent){
        ...
        <Space direction="vertical">
          ...
          <Typography.Text>To make this version "LIVE" please save your changes and press <CaretRightOutlined component="span" style={{verticalAlign:0, margin:"0 3px"}} /> button in the versions list</Typography.Text>
        </Space>
        ),
          ...NOTIFICATION_CONF
        });
        ...
        notification.info({
          ...
          description:(
            <Space direction="vertical">
              <Typography.Text> You are in <Tag color='orange'>Read-Only</Tag>mode.</Typography.Text>
              <Typography.Text>In Read-Only mode, you can click on assets and see descriptions but editing is disabled.</Typography.Text>
              <Typography.Text> To edit, click  <CloudDownloadOutlined component="span" style={{verticalAlign:0, margin:"0 3px"}}/> button on toolbar </Typography.Text>
            </Space>
          ),
         ...NOTIFICATION_CONF
        });
  ...
  const promptVersionLive =(e ,action, version) =>{
    ...
    if (action === 'pause'){
      confirm({
        ...
        okType: 'danger',
        onOk() { return setVersionLive(version, action) },
        onCancel() {},
      });
    } else {
      confirm({
        ...
        <Space style={{fontSize: "13px"}} size='small' split={<Divider type='vertical' />}>
          ...
          {version.isLive ? 
            <>
              ...
              <Typography.Text strong type='success'> LIVE </Typography.Text>
            </>
           : 
  ...
  const workingCopyData = (origin) =>{
    return (
      <div>
        <Space split={<Divider type='vertical' />}>
          <Typography.Text style={{display:'block'}} type='secondary'>Created At: {origin.createdAt || ''}</Typography.Text>
          <Typography.Text style={{display:'block'}} type='secondary'>Updated At: {origin.updatedAt || ''}</Typography.Text>
        </Space>
      </div>
    )
  }
  ...
  const getVersionsList = () => {
    if (versions.length === 0) {
      ...
      return (
        <Menu>
        {versions.map((version, index) => createMenuItem(version))}
        <Menu.Divider />
...
<>
  ...
  {!clickedVersion.name ? (
    <Item name='current_version'> <Badge color='#3bb44a' /> You are on Working Copy </Item>
  ) : (
    <>
      ...
      />
      <Item name='return_to_working_copy' icon={<RollbackOutlined />} tooltip={'Go to "Working Copy"'} onClick={getWorkingCopy} />
    </>
  )}
