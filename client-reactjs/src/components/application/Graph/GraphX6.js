...
function GraphX6({ readOnly = false, monitoring, statuses }) {
  ...
  const deleteAssetFromDataFlow = async (nodeData, _dataflowId) => {
    // Delete from any scheduled records after deleted node.
    const dependentNodes = graphRef.current
      .getNodes()
      .filter((node) => node.data?.schedule?.dependsOn?.includes(nodeData.assetId));
  ...
  useEffect(() => {
    ...
    (async () => {
          ...
          if (data?.graph) {
            ...
            if (!monitoring) {
              ...
              notification.success({
                message: `Working Copy is up to date with "LIVE" version "${data.name}"`,
                ...NOTIFICATION_CONF,
              });
            }
          } else {
            if (!monitoring) {
              ...
              notification.success({
                ...
                description:
                  'There is no "LIVE" version currently congfigured, check the versions list to find different versions available',
                ...NOTIFICATION_CONF,
              });
            }
          }
      ...
      graph.on('node:dblclick', ({ node, _e }) => {
      ...
      if (!readOnly) {
        ...
        graph.on('node:moved', async ({ _cell, _options }) => {
          // console.log('node:moved')
          handleSave(graph);
        });

        graph.on('node:added', async ({ _node, _index, _options }) => {
          console.log('node:added');
          handleSave(graph);
        });

        graph.on('node:change:data', async ({ _node, options }) => {
        ...
        graph.on('node:change:visible', async ({ _node, options }) => {
          if (options.name === 'update-asset') {
        ...
        graph.on('edge:added', async ({ _edge, _index, _options }) => {
          handleSave(graph);
        });
            ...
            if (sourceCell.data.type === 'File' && targetCell.data.type === 'File') {
              if (sourceCell.data.isSuperFile || targetCell.data.isSuperFile) {
                ...
                const superFileEdgeStyle = {
                  line: {
                    targetMarker: { fill: colors.superFileArrow, stroke: colors.superFileArrow, name: 'block' },
                    stroke: colors.superFileArrow,
                    strokeDasharray: 0,
                    strokeWidth: '2',
                  },
                };
  ...
  useEffect(() => {
    if (graphReady && readOnly === true && statuses?.length > 0) {
      ...
      let nodes = graphRef.current
        .getNodes()
        .filter((node) => node.data.type === 'Job' || node.data.type === 'Sub-Process');
      nodes.forEach((node) => {
        const children = node.getChildren();
        if (children) {
  ...
  const addRelatedFiles = (relatedFiles, cell) => {
    // 1. get all files,
    const allFiles = graphRef.current
      .getNodes()
      .filter((node) => node.data.type === 'File' || node.data.type === 'FileTemplate');
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      const { _files, jobs, _edges } = cellsJSON;
      // console.log('-data-----------------------------------------');
    ...
    if (configDialog.assetId || (!configDialog.assetId && configDialog.type === 'Sub-Process')) {
      ...
      return (
        <AssetDetailsDialog
          ...
          selectedAsset={{
            // all we know about clicked asset will be passed in selectedAsset prop
