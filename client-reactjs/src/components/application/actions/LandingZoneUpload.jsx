...
useEffect(() => {
  const url = process.env.NODE_ENV === 'development' ? devURL :  prodURL;
    ...
    socket.on('file-upload-response', (response => {
      ...
      socket.io.on("error", (error) => {
       ...
       return () => {socket.close(); message.destroy()}
  }, []);
  ...
  const handleFileUpload = () =>{
    ...
    if(!cluster){
      ...
      setUploading(true);
      setFiles(prev => (prev.map(item => { 
      item.uploadStatus = 'uploading'; return item})));
 ...
 //Dragger props
  const props = {
    ...
    onChange(info) {
      ...
      setFiles(() =>info.fileList.slice(-5).map(file => {file.key = file.uid; return file;})); //Causing issue -> file list should be used
     if (status === 'error') {
        message.error('Error adding files - please try again')
      }
    },
  };
  ...
  fetch(`/api/hpcc/read/getDropzones?clusterId=${selectedCluster.id}&for=fileUpload`,{
    ...
    handleError(response);
      ...
      setOptions(() => data.map(item => {return({'value' : item.path, 'label' : item.name, machine: item.machines[0], isLeaf: false, selectedCluster})}))       
  }).catch(err =>{
      console.log(err)
  })
  }
  ...
  const loadData = (selectedOptions) => {
    ...
    // Everytime user clicks an option on cascader make a call to fetch children
...
const handleFileDelete = (fileId) =>{
  setFiles(prev => (prev.filter(file => {return file.uid !== fileId})))
}
...
const columns = [
    ...
    return (
        ...
        style={{display : uploading ? 'none' : 'block', marginTop: '10px'}}
        >
        ...
        <div  style={{display : files.length > 0 ? "block" : "none"}}>
          <Table   columns={columns} 
            dataSource={files} size="small" 
            pagination={false} 
            style={{width: "100%", maxHeight : "300px", overflow: "auto"}}
            />
        </div>

        <Checkbox onChange={onCheckBoxChnage} style={{ margin : "20px 0px 20px 0px", display : uploading ? 'none' : 'block'}}>Overwrite File(s)</Checkbox>
