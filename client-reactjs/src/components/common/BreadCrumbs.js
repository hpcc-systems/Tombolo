...
class BreadCrumbs extends Component {
  render() {
    const getBreadCrumbs = () => {
      ...
      else if (location.pathname.includes('/shareApp')) path = 'share an application';
      else if (location.pathname.includes('/chart')) path = 'report';
      else path = pathSnippets[2];
      if (this.props.applicationId && this.props.applicationId !== '') {
        breadCrumbItems.push(
          <Breadcrumb.Item key={this.props.applicationTitle}>{this.props.applicationTitle}</Breadcrumb.Item>
        );
