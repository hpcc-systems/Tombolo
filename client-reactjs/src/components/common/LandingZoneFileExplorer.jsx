...
function LandingZoneFileExplorer({clusterId, DirectoryOnly, setLandingZoneRootPath, enableEdit, onDirectoryPathChange}) {
  ...
  const handleMachineChange = async(value) =>{
    ...
    try{
      const response = await fetchDirectories({clusterId, machine : value, path : `${landingZoneDetails.selectedLandingZone.path}` , dirOnly: DirectoryOnly});
  ...
  const loadCascaderData = async (selectedOptions) =>{
    try{
      ...
      const response = await fetchDirectories({clusterId, machine : landingZoneDetails.selectedMachine, path , dirOnly: DirectoryOnly});
      if(response?.FileListResponse?.files?.PhysicalFileStruct){
        const childDirs = response?.FileListResponse?.files?.PhysicalFileStruct.map(dir =>({label: dir.name,value : dir.name,isLeaf: false}));
  ...
  return (
    ...
    <Form.Item label='Landing Zone / Machine' required>
      <Input.Group compact >
          <Form.Item  style={{ width: '50%' }} name="landingZone" 
            ...
            <Select
              ...
              >
                {landingZoneDetails.landingZones.map(lz => (<Option key={lz.name} >{lz.name}</Option>))}
              </Select>
          </Form.Item>

          <Form.Item style={{ width: '50%' }} name="machine"
            ...
            <Select 
                ...
                style={{ width: '100%' }}
              >
                {landingZoneDetails.selectedLandingZone.machines.map(machine => <Option key={machine.Netaddress}> {machine.Netaddress}</Option>)}
              </Select>
          </Form.Item> 
      </Input.Group>
    </Form.Item> 
    ...
    {enableEdit ?
    <Form.Item label='Directory'  name="dirToMonitor" 
