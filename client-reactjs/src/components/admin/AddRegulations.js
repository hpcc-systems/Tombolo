...
class AddRegulations extends Component {
  ...
  SaveDetails() {
    ...
    this.state.selectedRowKeys.forEach(function (item) {
  ...
  handleOk = () => {
    var exists =
      this.state.regulations.filter((key) => key.compliance.toUpperCase() == this.state.compliance.toUpperCase())
        .length > 0;
    if (this.state.compliance == '') {
      ...
      (this.props.compliance != '' &&
        this.props.compliance.toUpperCase() != this.state.compliance.toUpperCase() &&
        exists) ||
