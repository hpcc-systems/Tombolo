...
function ScheduledJobsPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'x',
      render: (record) => <Space>
        <Button onClick={() => changeStatus('stop', record)} type="dashed">Stop</Button>
        <Button onClick={() => changeStatus('start', record)} type="primary">Start</Button>
        <Button danger onClick={() => removeJob(record)}>Remove</Button>
      </Space>,
    },
  ];
