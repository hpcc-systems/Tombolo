...
interface Props {
  basicUserDetailsForm: FormInstance;
  selectedUser: any;
  setUnsavedFields: (updater: (prev: any) => any) => void;
}

const EditUserBasicTab: React.FC<Props> = ({ basicUserDetailsForm, selectedUser, setUnsavedFields }) => {
  useEffect(() => {
    if (selectedUser) {
    ...
    if (selectedUser) {
      Object.keys(changedValues).forEach(key => {
        if (selectedUser[key] !== changedValues[key]) {
          setUnsavedFields((prev: any) => ({ ...prev, userDetails: [...prev.userDetails, key] }));
        } else {
          setUnsavedFields((prev: any) => ({
            ...prev,
            userDetails: prev.userDetails.filter((field: any) => field !== key),
          }));
        }
      });
    }
