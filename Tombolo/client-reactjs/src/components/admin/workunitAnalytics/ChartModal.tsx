...
const ChartModal = ({ visible, onClose, data }: ChartModalProps) => {
  ...
  const chartData = useMemo((): ChartDataPoint[] => {
    ...
    const aggregated = new Map<string, { x: string; y: number; count: number }>();
    ...
    let result: ChartDataPoint[] = Array.from(aggregated.values()).map(entry => ({
      x: entry.x,
  ...
  const buildTooltip = () => ({
    title: (d: ChartDataPoint) => String(d.x ?? ''),
    items: [
      {
        field: 'y',
        name: yAxis || 'value',
  ...
  const buildPieTooltip = () => ({
    title: (d: ChartDataPoint) => String(d.category ?? d.x ?? ''),
    items: [
      {
        field: 'value',
        name: yAxis || 'value',
          ...
          <Column
            ...
            label={
              chartData.length <= 15
                ? {
                    position: 'top',
                    formatter: (d: { y?: number }) => (d?.y != null ? String(d.y) : ''),
                  }
                : false
            }
          />
        ...
        return (
          <Pie
            ...
            statistic={{
              title: { content: 'Total' },
              content: {
                formatter: (_datum: unknown, data: ChartDataPoint[] | undefined) => {
