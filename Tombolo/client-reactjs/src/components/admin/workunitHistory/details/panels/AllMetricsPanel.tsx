...
interface RangeFilter {
  TimeElapsed: [number | undefined, number | undefined];
  NumRowsProcessed: [number | undefined, number | undefined];
  PeakMemoryUsage: [number | undefined, number | undefined];
}
...
const AllMetricsPanel: React.FC<Props> = ({ wu, details }) => {
  ...
  const [visibleCols, setVisibleCols] = useState<string[]>(loadLocalStorage('wuh.metrics.visibleCols', defaultVisible));
  const [groupMode, setGroupMode] = useState<GroupMode>(loadLocalStorage('wuh.metrics.groupMode', 'flat'));
  ...
  const onRangeChange = (key: keyof RangeFilter, idx: 0 | 1, val: number | null) => {
    setRange(r => ({
      ...r,
      [key]: idx === 0 ? [val ?? undefined, r[key][1]] : [r[key][0], val ?? undefined],
    }));
  };
  ...
  const filtered = useMemo(() => {
    ...
    const byFlat = (d: any) => {
      ...
      const inRange = (val: any, [min, max]: [number | undefined, number | undefined]) =>
        (min == null || (val ?? 0) >= min) && (max == null || (val ?? 0) <= max);
    ...
    const clone = (node: any): any => ({ ...node, children: node.children?.map(clone) || [] });
  ...
  const columns = useMemo(() => {
    ...
    if (visibleCols.includes('TimeTotalExecute'))
      base.push({
        ...
        align: 'right' as const,
        render: (v: any) => <span className={styles.numericText}>{formatSeconds(v)}</span>,
        sorter: (a: any, b: any) => (a.TimeTotalExecute || 0) - (b.TimeTotalExecute || 0),
      });
    ...
    if (visibleCols.includes('SizeDiskRead'))
      base.push({
        ...
        sorter: (a: any, b: any) => (a.SizeDiskRead || 0) - (b.SizeDiskRead || 0),
      });
    if (visibleCols.includes('SizeDiskWrite'))
      base.push({
        ...
        align: 'right' as const,
        render: (v: any) => <span className={styles.numericText}>{formatBytes(v)}</span>,
        sorter: (a: any, b: any) => (a.SizeDiskWrite || 0) - (b.SizeDiskWrite || 0),
      });
  ...
  const dataSource = useMemo(() => {
    if (groupMode === 'flat') {
      return filtered.map((d: any, idx: number) => ({ key: d.scopeId || d.scopeName || idx, ...d }));
    }
    const markKeys = (nodes: any[]): any[] =>
      nodes.map((n, idx) => ({
