...
interface EditClusterModalProps {
  displayEditClusterModal: boolean;
...
const EditClusterModal: React.FC<EditClusterModalProps> = ({
  ...
  const [updateCredentials, setUpdateCredentials] = useState<boolean>(false);
  const [abortController, setAbortController] = useState<AbortController | null>(null);
  const [allowSelfSigned, setAllowSelfSigned] = useState<boolean>(Boolean(selectedCluster?.allowSelfSigned));
  ...
  const handleClusterUpdate = async () => {
    ...
    try {
      ...
      const updatedInfo = await clustersService.update({ id: selectedCluster?.id as string, clusterInfo: formValues });
