...
import { Modal, Button, Form, Input, Radio, Card, Descriptions, Tooltip, Typography, FormInstance } from 'antd';
...
interface AddApplicationProps {
  ...
  selectedApplication: ApplicationUI | null;
  user: User;
...
const AppDescription: FC<AppDescriptionProps> = ({ application }) => {
  const formatDate = (date: string): string => (date ? dayjs(date).format('MMMM Do YYYY, h:mm:ss a') : 'N/A');
  ...
  return (
    <Descriptions bordered size="small" column={1}>
      ...
      <Descriptions.Item label="Created At">
        <Text text={formatDate(application.createdAt as string)} />
      </Descriptions.Item>
      <Descriptions.Item label="Updated At">
        <Text text={formatDate(application.updatedAt as string)} />
      </Descriptions.Item>
    </Descriptions>
  );
};
...
const FormContent: FC<FormContentProps> = ({ form, mode }) => {
...
const AddApplication: FC<AddApplicationProps> = ({
  ...
  const validateForms = async (): Promise<{ validationError: any; formData: FormValues | {} }> => {
  ...
  const saveApplication = async (): Promise<void> => {
    ...
    try {
      ...
      const title = (responseData as any).title;
      const description = (responseData as any).description;
      const userAppId = (responseData as any).user_app_id ?? (responseData as any).userAppId ?? null;
      if (userAppId) {
        user.applications.push({ id: userAppId, application: { id, title, description } });
      }
