...
export const findUniqueName = (name: string, clusterMonitoring: ClusterMonitoringDTO[]): string => {
  let i = 1;
  let newName = name + ' ( ' + i + ' )';
...
interface IdentifyErroneousTabsParams {
  erroneousFields: string[];
}

export const identifyErroneousTabs = ({ erroneousFields }: IdentifyErroneousTabsParams): string[] => {
