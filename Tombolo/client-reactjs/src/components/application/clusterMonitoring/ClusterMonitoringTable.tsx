...
interface DuplicatingData {
  ...
  selectedMonitoring?: ClusterMonitoringDTO;
}

interface ClusterMonitoringTableProps {
  clusterMonitoring: ClusterMonitoringDTO[];
  applicationId: string;
  setSelectedMonitoring: (monitoring: ClusterMonitoringDTO | null) => void;
  isReader: boolean;
  setDisplayViewDetailsModal: (display: boolean) => void;
  setDisplayAddEditModal: (display: boolean) => void;
  setEditingMonitoring: (editing: boolean) => void;
  setApproveRejectModal: (display: boolean) => void;
  selectedRows: ClusterMonitoringDTO[];
  setSelectedRows: (rows: ClusterMonitoringDTO[]) => void;
...
function ClusterMonitoringTable({
...
}: ClusterMonitoringTableProps) {
  //Actions
  const editClusterMonitoring = (record: ClusterMonitoringDTO) => {
  ...
  const evaluateMonitoring = (record: ClusterMonitoringDTO) => {
    setSelectedMonitoring(record);
    setApproveRejectModal(true);
  };

  // When the copy/duplicate icon is clicked
  const duplicateClusterMonitoring = (record: ClusterMonitoringDTO) => {
  ...
  const toggleMonitoringStatus = async (record: ClusterMonitoringDTO, action: 'start' | 'pause') => {
    try {
  ...
  const viewDetails = (monitoringId: string) => {
    setDisplayViewDetailsModal(true);
    const selectedMonitoring = clusterMonitoring.find(monitoring => monitoring.id === monitoringId);
    setSelectedMonitoring(selectedMonitoring || null);
  };

  //Columns
  const columns: ColumnsType<ClusterMonitoringDTO> = [
    { title: 'Name', dataIndex: 'monitoringName' },
    {
      title: 'Cluster',
      dataIndex: 'cluster_id',
      render: (_: any, record: ClusterMonitoringDTO) => {
        return (
    ...
    },
    {
      ...
      render: (type: string[]) => type.map(t => <Tag key={t}>{t}</Tag>),
    },
    { title: 'Active', dataIndex: 'isActive', render: (isActive: boolean) => (isActive ? 'Yes' : 'No') },
    { title: 'Approval Status', dataIndex: 'approvalStatus', render: (status: string) => startCase(status) },
              ...
              <Popover
                ...
                content={
                  <div
                    ...
                    <Link
                      to={`/${applicationId}/dashboard/notifications?monitoringId=${record.id}&monitoringType=clusterMonitoring`}>
  ...
  return (
    <Table
      ...
      rowSelection={{
        type: 'checkbox',
        onChange: (_selectedRowKeys: React.Key[], selectedRowsData: ClusterMonitoringDTO[]) => {
