...
function ClusterMonitoring() {
  ...
  //Local State
  const [clusterMonitoring, setClusterMonitoring] = useState<ClusterMonitoringDTO[]>([]);
  const [displayAddEditModal, setDisplayAddEditModal] = useState(false);
  const [selectedMonitoring, setSelectedMonitoring] = useState<ClusterMonitoringDTO | null>(null);
  ...
  const [selectedRows, setSelectedRows] = useState<ClusterMonitoringDTO[]>([]);
  ...
  const [filteredClusterMonitoring, setFilteredClusterMonitoring] = useState<ClusterMonitoringDTO[]>([]);
  ...
  }, [editingMonitoring, duplicatingData, selectedMonitoring, form, clusterMonitoring]);
  ...
  const handleBulkStartPause = async ({ ids, action }: { ids: string[]; action: 'start' | 'pause' }) => {
  ...
  return (
    <>
      ...
      />
      {displayViewDetailsModal && selectedMonitoring && (
        <ViewDetailsModal
