...
function ViewDetailsModal({
  ...
  useEffect(() => {
    ...
    if (selectedMonitoring?.metaData?.asrSpecificMetaData?.productCategory) {
      ...
      setSelectedProductCategory(pc || null);
    }
  }, [selectedMonitoring, domains, productCategories]);
      ...
      <Descriptions column={1} bordered={true} size="small" className="tiny-description">
        ...
        {selectedMonitoring?.metaData?.contacts?.primaryContacts && (
          <Descriptions.Item label={<b>Primary Contacts</b>}>
            {selectedMonitoring.metaData.contacts.primaryContacts.map((contact: string) => (
        ...
        {selectedMonitoring?.metaData?.contacts?.secondaryContacts && (
          <Descriptions.Item label={<b>Secondary Contacts</b>}>
            {selectedMonitoring.metaData.contacts.secondaryContacts.map((contact: string) => (
              <Tag key={contact} color="blue">
                {contact}
              </Tag>
            ))}
        ...
        {selectedMonitoring?.metaData?.contacts?.notifyContacts && (
          <Descriptions.Item label={<b>Notify Contacts</b>}>
            {selectedMonitoring.metaData.contacts.notifyContacts.map((contact: string) => (
              <Tag key={contact} color="blue">
                {contact}
              </Tag>
            ))}
