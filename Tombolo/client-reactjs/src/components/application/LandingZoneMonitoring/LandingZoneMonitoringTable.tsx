...
const LandingZoneMonitoringTable = ({
  ...
  const columns = [
    {
      ...
      render: (text: string) => {
        return (
          <span
            style={{
              background: searchTerm.length > 0 && text.toLocaleLowerCase().includes(searchTerm) ? 'var(--highlight)' : 'transparent',
            }}>
            {text}
          </span>
        );
    ...
    { title: 'Landing Zone', dataIndex: 'metaData.monitoringData.dropzone', key: 'dropzone' },
    { title: 'Directory', dataIndex: 'metaData.monitoringData.directory', key: 'directory' },
    {
      title: 'Created By',
    ...
    { title: 'Active', dataIndex: 'isActive', key: 'active', render: (_: any, record: any) => (record.isActive ? 'Yes' : 'No') },
    { title: 'Approval status', dataIndex: 'approvalStatus', key: 'approvalStatus' },
    {
      title: 'Actions',
      key: 'actions',
      render: (_: any, record: any) => (
        <>
          ...
          {!isReader ? (
                  ...
                  <div className={styles.lzMonitoringTable__hidden_actions}>
                    ...
                    {record.isActive ? (
                      <div onClick={() => toggleMonitoringStatus(record)}>
                        <PauseCircleOutlined disabled={record.approvalStatus !== APPROVAL_STATUS.APPROVED} className={styles.icons} />
                        Pause
                      </div>
                    ) : (
                      <div onClick={() => toggleMonitoringStatus(record)}>
                        <PlayCircleOutlined disabled={record.approvalStatus !== APPROVAL_STATUS.APPROVED} className={styles.icons} />
                        Start
                      </div>
                    )}
                      ...
                      title={
                        <>
                          ...
                          <div style={{ maxWidth: 400 }}>
                            This action will delete all related data including notifications generated by this monitoring. If you would like to keep the data, you can deactivate the monitoring instead.
                          </div>
                        </>
                      }
            ...
            <Link to={`/${applicationId}/dashboard/notifications?monitoringId=124&monitoringType=landingZoneMonitoring`}>
              <BellOutlined style={{ marginRight: 15 }} />
            </Link>
          )}
        </>
  ...
  const editLandingZoneMonitoring = (record: any) => {
  ...
  const handleLzMonitoringDeletion = async (record: any) => {
    try {
      await landingZoneMonitoringService.delete(record.id);
      setLandingZoneMonitoring((prev: any[]) => prev.filter((lz) => lz.id !== record.id));
  ...
  const copyMonitoring = (record: any) => {
    setCopying(true);
    setSelectedMonitoring(record);
    setDisplayAddEditModal(true);
  };

  const toggleMonitoringStatus = async (record: any) => {
    try {
      ...
      setLandingZoneMonitoring((prev: any[]) => prev.map((lz) => (lz.id === record.id ? { ...lz, isActive: !lz.isActive } : lz)));
  ...
  return (
    <Table
      ...
      rowSelection={{
        type: 'checkbox',
        onChange: (_selectedRowKeys: any, selectedRowsData: any) => {
          setSelectedRows(selectedRowsData);
        },
      }}
      pagination={{ pageSize: 20 }}
      rowClassName={(record: any) => (record?.isActive ? commonStyles.table_active_row : commonStyles.table_inactive_row)}
    />
  );
};
