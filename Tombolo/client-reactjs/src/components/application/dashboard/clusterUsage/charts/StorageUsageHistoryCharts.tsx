...
interface Props {
  ...
  setViewExpandedGraph: (v: boolean) => void;
  setExpandedGraphData: (d: any) => void;
}

const StorageUsageHistoryCharts: React.FC<Props> = ({
  clusterUsageHistory,
  setViewExpandedGraph,
  setExpandedGraphData,
}) => {
  const [groupedClusterUsageHistory, setGroupedClusterUsageHistory] = useState<any[]>([]);
  ...
  useEffect(() => {
    if (clusterUsageHistory) {
      ...
      for (const engine in clusterUsageHistory) {
        ...
        if (Object.prototype.hasOwnProperty.call(groupedData, key)) groupedData[key].engines.push(engine);
        else groupedData[key] = { engines: [engine], data };
      }

      for (const key in groupedData) groupedHistory.push(groupedData[key]);
      setGroupedClusterUsageHistory(groupedHistory);
    }
  }, [clusterUsageHistory]);

  const handleViewMoreClicked = (data: any) => {
            ...
            <div className={styles.storageUsageHistoryCharts__chartAndName} key={index}>
              ...
              <div className={styles.storageUsageHistoryCharts__engineName}>
                ...
                <span className={styles.storageUsageHistoryCharts__engineName__additionalText}>
                  <Popover
                    content={clusterHistory.engines.map((item: any, idx: number) => (
                      <div key={idx}>{idx === 0 ? null : capitalize(item)}</div>
                    ))}>
