...
const BulkUpdateModal = ({
  ...
  const saveUpdate = async () => {
    ...
    try {
      ...
      // Update
      await handleBulkUpdateDirectoryMonitorings({ updatedData: updatedMetaData });
      const allUpdatedIds = updatedMetaData.map((data) => data.id);
      const newDirectoryMonitoringData = directoryMonitorings.map((directory) => {
        if (allUpdatedIds.includes(directory.id)) {
          const updatedDirectory = updatedMetaData.find((data) => data.id === directory.id);
          return { ...directory, metaData: updatedDirectory.metaData };
        }
        return directory;
      });
      ...
      setDirectoryMonitorings(newDirectoryMonitoringData);
      resetState();
    } catch (err) {
