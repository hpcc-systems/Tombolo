...
function CostMonitoring() {
  ...
  const [filteredCostMonitoring, setFilteredCostMonitoring] = useState<CostMonitoringDTO[]>([]); // Filtered cost monitorings
  const [displayMonitoringDetailsModal, setDisplayMonitoringDetailsModal] = useState(false);
  const [selectedMonitoring, setSelectedMonitoring] = useState<CostMonitoringDTO | null>(null);
  const [editingData, setEditingData] = useState<EditingData>({ isEditing: false }); // Data to be edited
  const [duplicatingData, setDuplicatingData] = useState<DuplicatingData>({ isDuplicating: false }); // CM to be duplicated
  ...
  const [selectedClusters, setSelectedClusters] = useState<string[]>([]); // Selected clusters for cost monitoring
  const [activeTab, setActiveTab] = useState('0');
  const [selectedRows, setSelectedRows] = useState<CostMonitoringDTO[]>([]);
  ...
  useEffect(() => {
    ...
    let filteredCm = costMonitorings.filter(costMonitoring => {
      ...
      if (filterClusters && filterClusters.length > 0) {
        const hasMatchingCluster = costMonitoring.clusterIds?.some((clusterId: string) =>
          filterClusters.includes(clusterId)
        );
      ...
      if (filterUsers && filterUsers.length > 0) {
        ...
        const hasMatchingUser = filterUsers.some((filterUser: string) => costMonitoringUsers.includes(filterUser));
        if (!hasMatchingUser) {
          include = false;
        }
      }
    ...
    try {
      ...
      for (let key in notificationSpecificFields) {
        if ((notificationSpecificFields as any)[key] !== undefined) {
          notificationMetaData[key] = (notificationSpecificFields as any)[key];
        }
        delete allInputs[key];
      }
      ...
      handleError(err?.message || 'Failed to save cost monitoring');
    } finally {
      setSavingCostMonitoring(false);
    }
  };
  ...
  const handleUpdateCostMonitoring = async () => {
    ...
    try {
      ...
      setCostMonitorings(prev => {
        ...
        const updatedArray = Array.isArray(responseData?.data)
          ? responseData.data
          : [responseData?.data].filter(Boolean);
        if (updatedArray.length === 0) return prev;

        const updatedMap = new Map(updatedArray.map((u: any) => [u.id, u]));
  ...
  const handleBulkDeleteSelectedCostMonitorings = async (ids: string[]) => {
    try {
  ...
  const handleBulkStartPauseCostMonitorings = async ({ ids, action }: { ids: string[]; action: string }) => {
    try {
      const response = await costMonitoringService.toggle({ ids, action: action as any });
      const updatedMonitorings = response.updatedCostMonitorings;
      const updatedMap = new Map(updatedMonitorings.map((u: any) => [u.id, u]));
        ...
        <MonitoringDetailsModal
          ...
          {selectedMonitoring.metaData?.users && selectedMonitoring.metaData.users.length > 0 && (
            <Descriptions.Item label="Monitored Users">
              {selectedMonitoring.metaData.users.map((monitoredUser: string, index: number) => (
