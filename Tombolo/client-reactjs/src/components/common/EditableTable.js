      ...
      <Form ref={this.formRef}>
        <Form.Item
          ...
          {celleditor == 'select' ? (
            ...
            <Input ref={(node) => (this.input = node)} onBlur={this.saveText} />
          ) : (
            <TextArea ref={(node) => (this.input = node)} rows="5" cols="25" onBlur={this.saveText} />
          )}
        </Form.Item>
      </Form>
    ) : (
          ...
          {this.props.showDataDefinition && this.props.dataDefinitions ? (
            <span style={{ paddingRight: '5px' }}>
              <Select
                ...
                style={{ width: 230 }}>
                {this.props.dataDefinitions.map((dataDefn) => (
                  <Option key={dataDefn.id}>{dataDefn.name}</Option>
                ))}
              </Select>
            </span>
          ) : null}
