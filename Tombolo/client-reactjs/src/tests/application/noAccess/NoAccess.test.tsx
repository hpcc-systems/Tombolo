...
describe('NoAccess', () => {
  ...
  describe('Access Request Submission', () => {
    it('submits access request successfully', async () => {
      const user = userEvent.setup();
      (authService.requestAccess as any).mockResolvedValue({ success: true });
    ...
    it('handles error when user is not found', async () => {
      const user = userEvent.setup();
      (getUser as any).mockReturnValue(null);
    ...
    it('handles request access API error', async () => {
      ...
      (authService.requestAccess as any).mockRejectedValue({ message: errorMessage });
    ...
    it('handles multiple error messages from API', async () => {
      ...
      (authService.requestAccess as any).mockRejectedValue({ messages: errorMessages });
  ...
  describe('Check Access Status', () => {
    it('successfully checks and grants access when user has roles and applications', async () => {
      ...
      (apiClient.get as any).mockResolvedValue({ data: updatedUser });
      (authService.refreshToken as any).mockResolvedValue({ success: true });
      ...
      await waitFor(
        () => {
          expect((window as any).location.replace).toHaveBeenCalledWith('/');
        },
        { timeout: 2000 }
      );
    });
    ...
    it('handles API error when checking access status', async () => {
      ...
      (apiClient.get as any).mockRejectedValue({ message: errorMessage });
    ...
    it('handles multiple error messages from check access API', async () => {
      const user = userEvent.setup();
      const errorMessages = ['Error 1', 'Error 2'];
      (apiClient.get as any).mockRejectedValue({ messages: errorMessages });
  ...
  describe('Loading States', () => {
    it('shows loading state on check access button while checking', async () => {
      ...
      (apiClient.get as any).mockReturnValue(getUserPromise);
