...
import {
  toggleJobMonitoringStatus,
  handleDeleteJobMonitoring,
} from '@/components/application/jobMonitoring/jobMonitoringUtils';
...
describe('JobMonitoringTable', () => {
  ...
  it('shows error when starting non-approved; toggles when approved', async () => {
    ...
    await waitFor(() => toggleJobMonitoringStatus.mock.calls.length > 0);
  ...
  it('deletes a monitoring via Popconfirm and updates state', async () => {
    ...
    await waitFor(() => handleDeleteJobMonitoring.mock.calls.length > 0);
    expect(handleDeleteJobMonitoring).toHaveBeenCalledWith({
      id: rowApproved.id,
      jobMonitorings: [rowApproved],
      setJobMonitorings: expect.any(Function),
    });
