...
describe('useMonitoringsAndAllProductCategories', () => {
  ...
  it('shows error messages on failures', async () => {
    ...
    await waitFor(() => (message as any).error.mock.calls.some(call => call[0] === 'Error fetching job monitorings'));
    await waitFor(() =>
      (message as any).error.mock.calls.some(call => call[0] === 'Error fetching list of all products categories')
    );
  });
