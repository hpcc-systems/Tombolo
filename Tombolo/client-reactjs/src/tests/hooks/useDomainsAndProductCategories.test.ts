...
describe('useDomainAndCategories', () => {
  ...
  it('handles errors by showing message.error for domains and product categories', async () => {
    ...
    await waitFor(() => (message as any).error.mock.calls.some(call => call[0] === 'Error fetching domains'));
    ...
    await waitFor(() => (message as any).error.mock.calls.some(call => call[0] === 'Error fetching product category'));
  });
});
