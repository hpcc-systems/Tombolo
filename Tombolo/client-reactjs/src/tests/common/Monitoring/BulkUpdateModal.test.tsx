...
import BulkUpdateModal from '@/components/common/Monitoring/BulkUpdateModal';
...
vi.mock('antd', async importOriginal => {
  ...
  return { ...(antd as any), Modal: MockModal, Select: MockSelect, message, notification };
});
...
const baseProps = () => ({
  ...
  monitoringType: 'job',
  handleBulkUpdateMonitorings: vi.fn().mockResolvedValue(undefined),
});
...
describe('BulkUpdateModal', () => {
  ...
  it('shows error when handler rejects and keeps modal open', async () => {
    ...
    await waitFor(() => {
      ...
      const errorCall = (notification as any).error.mock.calls[0][0];
