...
describe('MonitoringDetailsModal', () => {
  ...
  test('renders audit info with creator, approver, and updater', async () => {
    ...
    expect(within(createdBySection as HTMLElement).getByText('John Doe')).toBeInTheDocument();
    expect(
      within(createdBySection as HTMLElement).getByText(content => content.includes('September 1, 2025'))
    ).toBeInTheDocument();

    const approvedByLabel = screen.getByText('Approved by');
    const approvedBySection = approvedByLabel.closest('.ant-descriptions-row');
    expect(within(approvedBySection as HTMLElement).getByText('Jane Smith')).toBeInTheDocument();
    expect(
      within(approvedBySection as HTMLElement).getByText(content => content.includes('September 2, 2025'))
    ).toBeInTheDocument();

    const updatedByLabel = screen.getByText('Last updated by');
    const updatedBySection = updatedByLabel.closest('.ant-descriptions-row');
    expect(within(updatedBySection as HTMLElement).getByText('Bob Johnson')).toBeInTheDocument();
    expect(
      within(updatedBySection as HTMLElement).getByText(content => content.includes('September 3, 2025'))
    ).toBeInTheDocument();
  });
