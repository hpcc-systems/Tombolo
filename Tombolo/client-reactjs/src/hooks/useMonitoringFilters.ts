...
interface UseMonitoringFiltersReturn {
  filterCount: number;
  setFilterCount: Dispatch<SetStateAction<number>>;
  clearFilters: () => void;
  handleFilterCountClick: () => void;
  handleDomainChange: (domainId: string) => void;
  handleFormChange: () => void;
  loadFilters: (
    filterOptions: FilterOptions,
    monitorings: MonitoringItem[],
    loadSpecificFilters?: (monitoring: MonitoringItem, updatedFilterOptions: FilterOptions) => void
  ) => FilterOptions;
}
 ...
 *
 * @param {FormInstance} form - The Ant Design Form instance used to manage filter form state.
...
const useMonitoringFilters = (
  ...
  lsKey: string,
  visibleLsKey: string
): UseMonitoringFiltersReturn => {
  const [filterCount, setFilterCount] = useState(0);

  const calculateFilterCount = useCallback((filters: Record<string, any>) => {
  ...
  }, [form, setFiltersVisible, calculateFilterCount, lsKey, visibleLsKey]);
  ...
  const debouncedHandleFormChange = debounce(
    (
      ...
      lsKey: string,
      setFilterCount: Dispatch<SetStateAction<number>>,
      calculateFilterCount: (filters: Record<string, any>) => number
    ) => {
