...
export function buildUserPrompt(context: AssistantRequestContext): string {
  return [
    ...
    'If Context says queries are already scoped by the server, treat those scope values as execution metadata only.',
    'When queries are already server-scoped, do NOT add WHERE predicates for the scope fields or values unless the user explicitly asks for them.',
    '',
    ...
    '  - UNIONs',
    '  - Semicolons',
    '  - When Context says the query is already scoped by the server: adding wuId/clusterId filters or any equivalent scope predicates',
    '',
