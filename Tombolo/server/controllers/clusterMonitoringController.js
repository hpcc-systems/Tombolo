...
const { cluster_monitoring, sequelize } = models;
...
const createClusterMonitoring = async (req, res) => {
  try {
    const newMonitoring = await cluster_monitoring.create({
      ...req.body,
    ...
    const monitoring = await cluster_monitoring.findOne({
...
const getAllClusterMonitoring = async (req, res) => {
  try {
    const monitoring = await cluster_monitoring.findAll({
      include: getCommonIncludes({ customIncludes: [] }),
      order: [['createdAt', 'DESC']],
    });
...
const updateClusterMonitoring = async (req, res) => {
  try {
    ...
    await cluster_monitoring.update(
      {
        ...req.body,
        isActive: false,
    ...
    const updatedMonitoring = await cluster_monitoring.findOne({
      where: { id: req.body.id },
      include: getCommonIncludes({ customIncludes: [] }),
    });
...
const evaluateClusterMonitoring = async (req, res) => {
  try {
    ...
    const updatedMonitoring = await cluster_monitoring.findAll({
      where: { id: { [Op.in]: ids } },
      include: getCommonIncludes({ customIncludes: [] }),
    });
  ...
  try {
    ...
    await Promise.all(
      monitoring.map(async data => {
        try {
          const monitoringRecord = await cluster_monitoring.findOne({
            where: { id: data.id },
          });
...
const deleteClusterMonitoring = async (req, res) => {
  ...
  try {
    ...
    await cluster_monitoring.destroy({
      where: { id: ids },
      transaction,
    });
