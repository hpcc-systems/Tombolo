// Imports from libraries
const Sequelize = require('sequelize');

// Local Imports
const logger = require('../config/logger');
const { OrbitProfileMonitoring, Cluster, sequelize } = require('../models');
const { APPROVAL_STATUS } = require('../config/constants');
const { sendError, sendSuccess } = require('../utils/response');
const { getUserFkIncludes } = require('../utils/getUserFkIncludes');
const {
  uniqueConstraintErrorHandler,
} = require('../utils/uniqueConstraintErrorHandler');

// Get all orbit profile monitorings for an application
const getAllOrbitProfileMonitorings = async (req, res) => {
  try {
    const { applicationId } = req.params;

    const orbitProfileMonitorings = await OrbitProfileMonitoring.findAll({
      where: { applicationId },
      include: [
        {
          model: Cluster,
          as: 'cluster',
          attributes: ['id', 'name', 'thor_host', 'thor_port'],
        },
        ...getUserFkIncludes(),
      ],
      order: [['createdAt', 'DESC']],
    });

    sendSuccess(res, orbitProfileMonitorings);
  } catch (err) {
    logger.error('Error fetching orbit profile monitorings:', err);
    sendError(res, 'Failed to fetch orbit profile monitorings');
  }
};

// Get orbit profile monitoring by ID
const getOrbitProfileMonitoringById = async (req, res) => {
  try {
    const { id, applicationId } = req.params;

    const orbitProfileMonitoring = await OrbitProfileMonitoring.findOne({
      where: { id, applicationId },
      include: [
        {
          model: Cluster,
          as: 'cluster',
          attributes: ['id', 'name', 'thor_host', 'thor_port'],
        },
        ...getUserFkIncludes(),
      ],
    });

    if (!orbitProfileMonitoring) {
      return sendError(res, 'Orbit profile monitoring not found', 404);
    }

    sendSuccess(res, orbitProfileMonitoring);
  } catch (err) {
    logger.error('Error fetching orbit profile monitoring:', err);
    sendError(res, 'Failed to fetch orbit profile monitoring');
  }
};

// Create new orbit profile monitoring
const createOrbitProfileMonitoring = async (req, res) => {
  try {
    const { name, clusterId, metaData, applicationId } = req.body;
    const userId = req.user.id;

    const newOrbitProfileMonitoring = await OrbitProfileMonitoring.create({
      applicationId,
      name,
      clusterId,
      metaData,
      createdBy: userId,
      lastUpdatedBy: userId,
      approvalStatus: APPROVAL_STATUS.PENDING,
      isActive: false,
    });

    const createdMonitoring = await OrbitProfileMonitoring.findByPk(
      newOrbitProfileMonitoring.id,
      {
        include: [
          {
            model: Cluster,
            as: 'cluster',
            attributes: ['id', 'name', 'thor_host', 'thor_port'],
          },
          ...getUserFkIncludes(),
        ],
      }
    );

    sendSuccess(
      res,
      createdMonitoring,
      'Orbit profile monitoring created successfully',
      201
    );
  } catch (err) {
    logger.error('Error creating orbit profile monitoring:', err);

    if (err instanceof Sequelize.UniqueConstraintError) {
      return uniqueConstraintErrorHandler(res, err);
    }

    sendError(res, 'Failed to create orbit profile monitoring');
  }
};

// Update orbit profile monitoring
const updateOrbitProfileMonitoring = async (req, res) => {
  try {
    const { id, applicationId } = req.params;
    const updateData = req.body;
    const userId = req.user.id;

    const orbitProfileMonitoring = await OrbitProfileMonitoring.findOne({
      where: { id, applicationId },
    });

    if (!orbitProfileMonitoring) {
      return sendError(res, 'Orbit profile monitoring not found', 404);
    }

    await orbitProfileMonitoring.update({
      ...updateData,
      lastUpdatedBy: userId,
      approvalStatus: APPROVAL_STATUS.PENDING, // Reset to pending on update
    });

    const updatedMonitoring = await OrbitProfileMonitoring.findByPk(id, {
      include: [
        {
          model: Cluster,
          as: 'cluster',
          attributes: ['id', 'name', 'thor_host', 'thor_port'],
        },
        ...getUserFkIncludes(),
      ],
    });

    sendSuccess(
      res,
      updatedMonitoring,
      'Orbit profile monitoring updated successfully'
    );
  } catch (err) {
    logger.error('Error updating orbit profile monitoring:', err);

    if (err instanceof Sequelize.UniqueConstraintError) {
      return uniqueConstraintErrorHandler(res, err);
    }

    sendError(res, 'Failed to update orbit profile monitoring');
  }
};

// Delete orbit profile monitoring (soft delete)
const deleteOrbitProfileMonitoring = async (req, res) => {
  try {
    const { id, applicationId } = req.params;
    const userId = req.user.id;

    const orbitProfileMonitoring = await OrbitProfileMonitoring.findOne({
      where: { id, applicationId },
    });

    if (!orbitProfileMonitoring) {
      return sendError(res, 'Orbit profile monitoring not found', 404);
    }

    await orbitProfileMonitoring.destroy({
      userId, // This will set deletedBy field through DeleteMixin
    });

    sendSuccess(res, null, 'Orbit profile monitoring deleted successfully');
  } catch (err) {
    logger.error('Error deleting orbit profile monitoring:', err);
    sendError(res, 'Failed to delete orbit profile monitoring');
  }
};

// Toggle orbit profile monitoring status
const toggleOrbitProfileMonitoringStatus = async (req, res) => {
  try {
    const { id, applicationId } = req.params;
    const { isActive, comment } = req.body;
    const userId = req.user.id;

    const orbitProfileMonitoring = await OrbitProfileMonitoring.findOne({
      where: { id, applicationId },
    });

    if (!orbitProfileMonitoring) {
      return sendError(res, 'Orbit profile monitoring not found', 404);
    }

    await orbitProfileMonitoring.update({
      isActive,
      lastUpdatedBy: userId,
      approverComment: comment || null,
    });

    const updatedMonitoring = await OrbitProfileMonitoring.findByPk(id, {
      include: [
        {
          model: Cluster,
          as: 'cluster',
          attributes: ['id', 'name', 'thor_host', 'thor_port'],
        },
        ...getUserFkIncludes(),
      ],
    });

    sendSuccess(
      res,
      updatedMonitoring,
      'Monitoring status updated successfully'
    );
  } catch (err) {
    logger.error('Error toggling orbit profile monitoring status:', err);
    sendError(res, 'Failed to update monitoring status');
  }
};

// Bulk update orbit profile monitorings
const bulkUpdateOrbitProfileMonitorings = async (req, res) => {
  try {
    const { applicationId } = req.params;
    const { ids, updates } = req.body;
    const userId = req.user.id;

    await OrbitProfileMonitoring.update(
      {
        ...updates,
        lastUpdatedBy: userId,
      },
      {
        where: {
          id: { [Sequelize.Op.in]: ids },
          applicationId,
        },
      }
    );

    const updatedMonitorings = await OrbitProfileMonitoring.findAll({
      where: {
        id: { [Sequelize.Op.in]: ids },
        applicationId,
      },
      include: [
        {
          model: Cluster,
          as: 'cluster',
          attributes: ['id', 'name', 'thor_host', 'thor_port'],
        },
        ...getUserFkIncludes(),
      ],
    });

    sendSuccess(
      res,
      updatedMonitorings,
      'Orbit profile monitorings updated successfully'
    );
  } catch (err) {
    logger.error('Error bulk updating orbit profile monitorings:', err);
    sendError(res, 'Failed to bulk update orbit profile monitorings');
  }
};

// Approve orbit profile monitoring
const approveOrbitProfileMonitoring = async (req, res) => {
  try {
    const { id, applicationId } = req.params;
    const { comment } = req.body;
    const userId = req.user.id;

    const orbitProfileMonitoring = await OrbitProfileMonitoring.findOne({
      where: { id, applicationId },
    });

    if (!orbitProfileMonitoring) {
      return sendError(res, 'Orbit profile monitoring not found', 404);
    }

    await orbitProfileMonitoring.update({
      approvalStatus: APPROVAL_STATUS.APPROVED,
      approvedBy: userId,
      approvedAt: new Date(),
      approverComment: comment || null,
      lastUpdatedBy: userId,
    });

    const updatedMonitoring = await OrbitProfileMonitoring.findByPk(id, {
      include: [
        {
          model: Cluster,
          as: 'cluster',
          attributes: ['id', 'name', 'thor_host', 'thor_port'],
        },
        ...getUserFkIncludes(),
      ],
    });

    sendSuccess(
      res,
      updatedMonitoring,
      'Orbit profile monitoring approved successfully'
    );
  } catch (err) {
    logger.error('Error approving orbit profile monitoring:', err);
    sendError(res, 'Failed to approve orbit profile monitoring');
  }
};

// Reject orbit profile monitoring
const rejectOrbitProfileMonitoring = async (req, res) => {
  try {
    const { id, applicationId } = req.params;
    const { comment } = req.body;
    const userId = req.user.id;

    const orbitProfileMonitoring = await OrbitProfileMonitoring.findOne({
      where: { id, applicationId },
    });

    if (!orbitProfileMonitoring) {
      return sendError(res, 'Orbit profile monitoring not found', 404);
    }

    await orbitProfileMonitoring.update({
      approvalStatus: APPROVAL_STATUS.REJECTED,
      approvedBy: userId,
      approvedAt: new Date(),
      approverComment: comment || null,
      lastUpdatedBy: userId,
    });

    const updatedMonitoring = await OrbitProfileMonitoring.findByPk(id, {
      include: [
        {
          model: Cluster,
          as: 'cluster',
          attributes: ['id', 'name', 'thor_host', 'thor_port'],
        },
        ...getUserFkIncludes(),
      ],
    });

    sendSuccess(
      res,
      updatedMonitoring,
      'Orbit profile monitoring rejected successfully'
    );
  } catch (err) {
    logger.error('Error rejecting orbit profile monitoring:', err);
    sendError(res, 'Failed to reject orbit profile monitoring');
  }
};

// Search for orbit builds (placeholder implementation)
const searchOrbitBuilds = async (req, res) => {
  try {
    const { searchTerm } = req.body;

    // Mock implementation - replace with actual orbit API integration
    const mockBuilds = [
      { label: 'Current Carrier Build', value: 'current_carrier' },
      { label: 'TN DMV Build', value: 'tn_dmv' },
      { label: 'Market Magnifier Build', value: 'market_magnifier' },
      { label: 'Insurance Analytics Build', value: 'insurance_analytics' },
      { label: 'Risk Assessment Build', value: 'risk_assessment' },
    ];

    const filteredBuilds = mockBuilds.filter(build =>
      build.label.toLowerCase().includes(searchTerm?.toLowerCase() || '')
    );

    sendSuccess(res, filteredBuilds);
  } catch (err) {
    logger.error('Error searching orbit builds:', err);
    sendError(res, 'Failed to search orbit builds');
  }
};

// Test orbit server connection (placeholder implementation)
const testOrbitConnection = async (req, res) => {
  try {
    const { orbitHost } = req.body;

    // Mock implementation - replace with actual orbit server connection test
    const connectionResult = {
      host: orbitHost,
      status: 'connected',
      responseTime: '45ms',
      message: 'Connection to Orbit server successful',
    };

    sendSuccess(res, connectionResult);
  } catch (err) {
    logger.error('Error testing orbit connection:', err);
    sendError(res, 'Failed to test orbit connection');
  }
};

module.exports = {
  getAllOrbitProfileMonitorings,
  getOrbitProfileMonitoringById,
  createOrbitProfileMonitoring,
  updateOrbitProfileMonitoring,
  deleteOrbitProfileMonitoring,
  toggleOrbitProfileMonitoringStatus,
  bulkUpdateOrbitProfileMonitorings,
  approveOrbitProfileMonitoring,
  rejectOrbitProfileMonitoring,
  searchOrbitBuilds,
  testOrbitConnection,
};
