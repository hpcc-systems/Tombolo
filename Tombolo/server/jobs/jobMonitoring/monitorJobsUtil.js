...
const inferWuStartTime = wuid => {
  ...
  // Regular expression to match YYYYMMDD-HHMMSS or YYYYMMDD-HHMM formats
  const regex = /^(\d{4})(\d{2})(\d{2})-(\d{2})(\d{2})(\d{2})?$/;
  const match = timestampStr.match(regex);

  if (!match) {
    throw new Error('Invalid WUID timestamp format');
  }
  ...
  if (isNaN(date.getTime())) {
    throw new Error('Invalid date parsed from WUID');
  }
