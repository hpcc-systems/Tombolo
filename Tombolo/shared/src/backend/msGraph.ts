...
class MSGraphClient {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    try {
      ...
      const data = (await response.json()) as MSGraphTokenResponse;
      ...
      logger.error(
        `Failed to obtain MS Graph access token: ${(error as Error).message}`
      );
      throw error;
    }
  }
  ...
  async getUserByUpnPrefix(upnPrefix: string): Promise<MSGraphUser | null> {
    ...
    try {
      ...
      const data = (await response.json()) as { value?: MSGraphUser[] };
      ...
      logger.error(
        `Failed to fetch user by UPN prefix '${upnPrefix}': ${(error as Error).message}`
      );
      return null;
    }
  }
  ...
  async getUserManager(emailOrUpn: string): Promise<MSGraphManager | null> {
    ...
    try {
      ...
      const manager = (await response.json()) as MSGraphManager;
      return manager;
    } catch (error) {
      logger.error(
        `Failed to fetch manager for '${emailOrUpn}': ${(error as Error).message}`
      );
      return null;
    }
  }
