...
(async () => {
    try {
      ...
      let logicalFileDetail = await hpccUtil.logicalFileDetails(
        Name,
        cluster_id
      );
      ...
      if (logicalFileDetail.Exception) {
        ...
        const fileDeleted =logicalFileDetail.Exception[0].Message.includes("Cannot find file");
      ...
      if(metaDifference.length > 0){ // Note - this does not cover file size not in range
